...
Here is Pallet Town, captured straight from the engine's `ascii` display adapter
(player + NPCs composited, the same 160×144 framebuffer the other adapters use —
each `ascii` cell is one 2×4-pixel block, `::` is the grass dither):
...
The `block`, `color`, and `braille` adapters draw the *same* bits at higher
fidelity (half-block / colored braille); `ascii` is the universal fallback shown
here because it survives a plain markdown code block.
